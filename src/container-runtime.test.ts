...
describe('readonlyMountArgs', () => {
  it('returns --mount flag with bind mount syntax', () => {
...
describe('stopContainer', () => {
  it('returns stop command string for valid container names', () => {
    const cmd = stopContainer('nanoclaw-test-123');
    expect(cmd).toBe(`${CONTAINER_RUNTIME_BIN} stop nanoclaw-test-123`);
  });
});
...
describe('ensureContainerRuntimeRunning', () => {
  ...
  it('starts runtime when status check fails, then throws if start also fails', () => {
    // system status fails
  ...
  it('starts runtime successfully when status check fails', () => {
    // system status fails
    mockExecSync.mockImplementationOnce(() => {
      throw new Error('not running');
    });
...
describe('cleanupOrphans', () => {
  ...
  it('ignores non-running and non-nanoclaw containers', () => {
    mockExecSync.mockReturnValueOnce(
      JSON.stringify([
        { status: 'stopped', configuration: { id: 'nanoclaw-old-1' } },
        { status: 'running', configuration: { id: 'other-app-2' } },
      ]),
    );
