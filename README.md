...
- **Per-SDK credential injection** — Claude OAuth (macOS keychain), Codex (호스트 `~/.codex/auth.json` 마운트), 채널 토큰(`.env`, 호스트 전용)
- **Local audio transcription** — Slack 음성/오디오를 WhisperX (faster-whisper + pyannote diarization + wav2vec2 align) 로 로컬 transcribe, speaker 라벨 포함해 agent 에 텍스트로 넘김
...
- Python 3.10+ + WhisperX venv (`data/whisperx-venv/`) + HF_TOKEN (pyannote 약관 동의 필요) — Slack audio transcription
...
이 포크는 원본에서 분기 후 패트/매트 dual bot · Codex SDK · whisper-cpp · 맞춤형 group 구조 등으로 독립 진화했고 upstream merge 는 더 이상 수행하지 않는다. 원본의 MIT 권리 고지는 [LICENSE](LICENSE) 참조.
