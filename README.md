...
**Small enough to understand.** One process, a few source files and no microservices. If you want to understand the full NanoClaw codebase, just ask Codex to walk you through it.
...
**AI-native.**
- No installation wizard; Codex guides setup.
- No monitoring dashboard; ask Codex what's happening.
- No debugging tools; describe the problem and Codex fixes it.

**Skills over features.** Instead of adding features (e.g. support for Telegram) to the codebase, contributors submit [Codex skills](https://developers.openai.com/codex/skills) like `/add-telegram` that transform your fork. You end up with clean code that does exactly what you need.

**Best harness, best model.** NanoClaw runs on the Codex SDK, which means you're running Codex directly. Codex is highly capable and its coding and problem-solving capabilities allow it to modify and expand NanoClaw and tailor it to each user.
...
```
Channels --> SQLite --> Polling loop --> Container (Codex SDK) --> Response
```
...
If you have issues, during setup, Codex will try to dynamically fix them. If that doesn't work, run `codex`, then run `/debug`. If Codex finds an issue that is likely affecting other users, open a PR to modify the setup SKILL.md.
