...
## 4. OpenAI Credentials via OneCLI
...
If an OpenAI secret is listed, confirm with user: keep or reconfigure? If keeping, skip to step 5.

AskUserQuestion: Do you have an **OpenAI API key**?

1. **Yes, I have one** — description: "You already have an API key from platform.openai.com."
2. **No, I need one** — description: "You'll need to create one at platform.openai.com/api-keys."
...
- CREDENTIALS=missing → re-run step 4 (check `onecli secrets list` for OpenAI secret)
...
**Container agent fails (Codex process exited with error):** Ensure the container runtime is running — `open -a Docker` (macOS Docker), `container system start` (Apple Container), or `sudo systemctl start docker` (Linux). Check container logs in `groups/main/logs/container-*.log`.
...
1. Use the Read tool to read `.agents/skills/setup/diagnostics.md`.
