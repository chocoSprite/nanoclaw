...
If both succeed, check for an OpenAI secret:

```bash
onecli secrets list
```

If an OpenAI secret exists, tell the user OneCLI is already configured and working. Use AskUserQuestion:
...
|---|---|---|
| `OPENAI_API_KEY` | `api_key` | `api.openai.com` |
...
```bash
onecli secrets create --name OpenAI --type api_key --value <key> --host-pattern api.openai.com
```

After successful migration, remove the credential lines from `.env`. Use the Edit tool to remove only the credential variable lines (`OPENAI_API_KEY`). Keep all other `.env` entries intact (e.g. `ONECLI_URL`, `TELEGRAM_BOT_TOKEN`, channel tokens).
...
Tell the user: "Migrated your OpenAI credentials from `.env` to the OneCLI Agent Vault. The raw keys have been removed from `.env` — they're now managed by OneCLI and will be injected at request time without entering containers."

### Offer to migrate other container-facing credentials

After handling OpenAI credentials (whether migrated or freshly registered), scan `.env` again for remaining credential variables that containers use for outbound API calls.
...
- One option per credential found (e.g., "PARALLEL_API_KEY" — description: "Used by Parallel AI integrations inside containers")
...
Check if OneCLI already has an OpenAI secret:
```bash
onecli secrets list
```

If an OpenAI secret already exists, skip to Phase 4.
...
1. **Dashboard** — description: "Best if you have a browser on this machine. Open http://127.0.0.1:10254 and add the secret in the UI. Use type 'api_key' and paste your key as the value."
...
**If the user's response happens to contain a key** (starts with `sk-` or looks like an API key): handle it gracefully — run the `onecli secrets create` command with that value on their behalf.

**After user confirms:** verify with `onecli secrets list` that an OpenAI secret exists. If not, ask again.
...
**Container gets no credentials:** Verify `ONECLI_URL` is set in `.env` and the gateway has an OpenAI secret (`onecli secrets list`).

**Old .env credentials still present:** This skill should have removed them. Double-check `.env` for `OPENAI_API_KEY` and remove it manually if still present.
